var Mrs = {
    $Mrs: {
        ...
        Connection: function( type, addr, port, timeout_msec ){
            ...
            self.Read = function( bytes ){
                if ( self.IsValidReadCallback() ){
                    self.OnRead( bytes, bytes.length );
                    return;
                }
            ...
            self.WriteRecord = function( record ){
            ...
            self.m_ReadCallback = 0;
            self.SetReadCallback = function( value ){
                self.m_ReadCallback = value;
            };
            self.IsValidReadCallback = function(){
                return ( 0 !== self.m_ReadCallback );
            };
            self.OnRead = function( data, data_len ){
                if ( 0 !== self.m_ReadCallback ) Runtime.dynCall( "viiii", self.m_ReadCallback, [ self.m_ConnectionId, self.m_Data, allocate( data, "i8", ALLOC_STACK ), data_len ] );
            };
            ...
            case Mrs.ConnectionType.WS:{
                self.m_Url = "ws://"+ addr +":"+ port;
            }break;
            
            case Mrs.ConnectionType.WSS:{
                self.m_Url = "wss://"+ addr +":"+ port;
            }break;
        ...
        ConnectionManager: {
            ...
            AddConnection: function( connection ){
                ...
                Mrs.ConnectionManager.s_ReadyConnections.push( connection );
            ...
            Update: function(){
                if ( 0 < Mrs.ConnectionManager.s_ReadyConnections.length ){
                    for ( var i = 0; i < Mrs.ConnectionManager.s_ReadyConnections.length; ++i ){
                        var connection = Mrs.ConnectionManager.s_ReadyConnections[ i ];
                        var socket = new WebSocket( connection.GetUrl() + connection.GetPath(), [] );
                        connection.SetSocket( socket );
                    }
                    Mrs.ConnectionManager.s_ReadyConnections = [];
                }
                ...
                Mrs.ConnectionManager.s_KeepAliveUpdateTimerId = setTimeout( function(){
                    ...
                    for ( var i = 0; i < connection_ids.length; ++i ){
                        ...
                        if ( connection.IsValidReadCallback() ) continue;
        ...
        mrs_set_read_callback: function( connection, _callback ){
            var connection_id = connection;
            connection = Mrs.ConnectionManager.GetConnection( connection_id );
            if ( null != connection ) connection.SetReadCallback( _callback );
        },
        ...
        mrs_connection_set_path: function( connection, value ){
            var connection_id = connection;
            connection = Mrs.ConnectionManager.GetConnection( connection_id );
            if ( null == connection ) return false;
            
            if ( typeof value !== "string" ) value = Pointer_stringify( value );
            connection.SetPath( value );
            return true;
        },
        
        mrs_connection_get_path: function( connection ){
            var connection_id = connection;
            connection = Mrs.ConnectionManager.GetConnection( connection_id );
            var value = ( null == connection ) ? "" : connection.GetPath();
        ...
        mrs_write: function( connection, data, data_len ){
            var connection_id = connection;
            connection = Mrs.ConnectionManager.GetConnection( connection_id );
            if ( null == connection ) return false;
            
            if ( ! ( data instanceof Uint8Array ) ) data = new Uint8Array( HEAPU8.buffer, data, data_len );
        ...
        mrs_get_error_string: function( error ){
            ...
            case Mrs.Error.ENOENT:          { msg = "MRS_ENOENT: No such file or directory"; }break;
            ...
            case Mrs.Error.ETIMEDOUT:       { msg = "MRS_ETIMEDOUT: Operation timed out"; }break;
        ...
        mrs_get_connection_error_string: function( error ){
            ...
            case Mrs.ConnectionError.WRITE_ERROR_MRU_OVER_MTU:        { msg = "MRS_WRITE_ERROR_MRU_OVER_MTU: Write error MRU over MTU"; }break;
            case Mrs.ConnectionError.WRITE_ERROR_MRU_SENDQ_FULL:      { msg = "MRS_WRITE_ERROR_MRU_SENDQ_FULL: Write error MRU sendq full"; }break;
        ...
        s_IsWebAssembly: false,
        IsWebAssembly: function(){
            return Mrs.s_IsWebAssembly;
        },
    },
    ...
    mrs_set_read_callback: function(){
        Mrs.mrs_set_read_callback.apply( this, arguments );
    },
    ...
    mrs_connection_set_path: function(){
        return Mrs.mrs_connection_set_path.apply( this, arguments );
    },
    
    mrs_connection_get_path: function(){
        return Mrs.mrs_connection_get_path.apply( this, arguments );
    },
    ...
    mrs_write: function(){
        return Mrs.mrs_write.apply( this, arguments );
    },
...
// [webgl only functions]
    __mrs_setup__: function(){
        ...
        Mrs.s_IsWebAssembly = "wasmMemory" in Module;
    },
...
// [unsupported functions]
    ...
    mrs_mru_set_client_peer_limit: function( value ){
        Mrs.LOG_ERR( "Unsupported function: mrs_mru_set_client_peer_limit" );
    },
    
    mrs_mru_get_client_peer_limit: function(){
        Mrs.LOG_ERR( "Unsupported function: mrs_mru_get_client_peer_limit" );
        return 0;
    },
    
    mrs_mru_set_sendq_size: function( value ){
        Mrs.LOG_ERR( "Unsupported function: mrs_mru_set_sendq_size" );
    },
    
    mrs_mru_get_sendq_size: function(){
        Mrs.LOG_ERR( "Unsupported function: mrs_mru_get_sendq_size" );
        return 0;
    },
};
autoAddDeps( Mrs, '$Mrs' );
